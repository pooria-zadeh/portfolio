...
export const MCheckboxFormik = (props: FormikCheckboxProps & { showOnError?: boolean }) => {
    return (
        <Field {...props}>
            {({ field, meta }: any) => <MCheckbox meta={meta} {...props} {...field} />}
        </Field>
    );
};

const WeekdayButton = styled(MText)<{ checked?: boolean }>(({ theme, checked }) => ({
    width: '3ch',
    height: '3ch',
    display: 'flex',
    alignItems:'center',
    ...
    ({
        ...
        return (
            <StyledCheckbox
                ...
                <WeekdayButton 
                variant="h2"
                checked={checked}>{optionName}</WeekdayButton>
                <input
                    {...props}
                    type="checkbox"
                    onChange={onChange}
...
export const WeekdayCheckboxFormik = memo(
    (props: FormikCheckboxProps & { showOnError?: boolean }) => {
        return (
            <Field {...props}>
                {({ field, meta }: any) => <WeekdayCheckbox meta={meta} {...props} {...field} />}
            </Field>
        );
    }
);
